...
int main(){
	...
	//Initializing some things ..
  	char * idPort = "5555";
  	int conexion_servidor, conexion_cliente, port, conexion_clienteBS[2]; //declaramos las variables
	...
	socklen_t longc2; //Debemos declarar una variable que contendrá la longitud de la estructura
	...
	while(noConexiones < 2){ //Empieza e cclo para establecer las conexiones con clientes
	...
	//mensaje = "HolaS";
	//int estado = sendto(conexion_cliente, mensaje, 100,0,inet_ntoa(ArraydeConexiones[0].sin_addr),0);
	//printf("\tEstado: %d\n", estado);
	...
	if(recv(conexion_cliente, buffer, 100, 0) < 0){ //Comenzamos a recibir datos del cliente
		...
		while(buffer != "chao"){
	    	...
	    	bzero((char *)&buffer, sizeof(buffer));
	    	recv(conexion_cliente, buffer, 100, 0);
	    	sendto(conexion_clienteBS[0], buffer, 100,0,inet_ntoa(ArraydeConexiones[1].sin_addr),0);
